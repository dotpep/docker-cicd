...
@app.route('/')
def hello_world():
    return 'Hello, Docker this is cool!'


@app.route('/widgets')
def get_widgets():
    with psycopg2.connect(host="db", user="postgres", password=password, database="python_docker_dev_example") as conn:
...
def db_init():
    ...
    with conn.cursor() as cur:
        cur.execute("DROP DATABASE IF EXISTS python_docker_dev_example")
        cur.execute("CREATE DATABASE python_docker_dev_example")
    conn.close()
